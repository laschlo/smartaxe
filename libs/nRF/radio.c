...
static void configure_registers()
{
	...
	// set Enhanced Shockburst retry to every 750us us, up to 15 times.  If packet collisions are a problem even with AA enabled,
...
void Radio_Init()
{
	...
	//Enable external interrupt for PD5
	DDRD &= ~_BV(PD5);
	PCICR |= _BV(PCIE2);
	PCMSK2 |= _BV(PCINT21);
...
uint8_t Radio_Resend_Rate()
{
	uint8_t reg;
	get_register(OBSERVE_TX,&reg,1);
	reg = reg & 0x0F;
	return reg;
}
